...
static void	free_cr_list(t_f **cr)
{
	free((*cr)->argument);
	free((*cr)->type);
	free((*cr)->modifier);
...
void		ft_printf4(t_f **cr, size_t *size)
{
...
size_t		ft_printf3(char **str, t_f **cr, int *i, int *nb_arg)
{
	int			j;
	size_t		size;
	t_f			*temp;
...
void		ft_printf2(char **str, size_t *size, t_f **cr, va_list ap)
{
	int		nb_arg;
	int		i;
