...
void	preci21(char *temp, t_f **cr, size_t i, size_t precision)
{
...
void	preci22(char *temp, t_f **cr, size_t precision, size_t size)
{
	size_t	i;
	size_t	j;
...
void	preci2(t_f **cr, size_t precision, size_t size, char *temp)
{
	...
	if (((*cr)->type[0] != 's' && (*cr)->type[0] != 'S' &&
				(*cr)->type[0] != 'c') && precision > size)
		preci22(temp, cr, precision, size);
	else if (precision < size && ((*cr)->type[0] == 's' ||
				(*cr)->type[0] == 'S'))
		preci21(temp, cr, i, precision);
	else if (((*cr)->precision[0] == '.' ||
				(*cr)->precision[0] == '0') &&
